"""Update handoff docs: change only CURRENT/CONFIGURED alpha references from 3e-15 to 5e-15.
...
patches = [
    ...
    (H / "thesis" / "alpha_sweep" / "cave_alpha_sweep.md", [
        ("Current alpha: `3e-15`", "Current alpha: `5e-15`"),
    ]),

    # data_contract.md
    (H / "data_contract.md", [
        ("current complex128/alpha=3e-15 rerun", "current complex128/alpha=5e-15 rerun"),
    ]),

    # thesis_evidence_summary.md
    (H / "thesis" / "reports" / "thesis_evidence_summary.md", [
        ("configured handoff value is `3e-15`", "configured handoff value is `5e-15`"),
    ]),

    # metric_audit_response.md
    (H / "thesis" / "reports" / "metric_audit_response.md", [
        ("ridge_alpha: 3.0e-15", "ridge_alpha: 5.0e-15"),
        ("complex128/alpha=3e-15", "complex128/alpha=5e-15"),
    ]),
...
with open(asj) as f:
    d = json.load(f)
d["current_alpha"] = 5e-15
d["current_alpha_label"] = "5e-15"
...
# Scan for any remaining "3e-15" as current/configured (not in sweep tables)
import re
for f in sorted(H.rglob("*.md")):
    ...
    for i, line in enumerate(text.splitlines(), 1):
        ...
        if stripped.startswith("|") and "e-15" in stripped:
            continue
        if "3e-15" in stripped and "current" in stripped.lower():
